...
function collapseDetails(event){
    ...
    const collapsable = caller.childNodes[1];
    const svg = caller.childNodes[0].childNodes[2].childNodes[1];
    ...
    if (collapsable.style.visibility === 'collapse'){
        collapsable.style.visibility = 'visible';
        path.setAttribute('d', downArrow);

    } else {
        collapsable.style.visibility = 'collapse';
